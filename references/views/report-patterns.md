...
## Document Reports vs Financial Ledger Reports (IFRS / GAAP Reporting Architecture)

Do not confuse single transactional document reports with statutory general ledger reporting:

| Report Category | Examples | Engine / Mechanism | Standard / Role |
|-----------------|----------|-------------------|-----------------|
| **Transactional Documents** | Customer Invoice, Vendor Bill, Delivery Slip, Purchase Order, Payment Receipt | QWeb Template (`ir.actions.report` with `qweb-pdf`) | Commercial documentation, tax compliance, legally binding external evidence. |
| **Statutory Financial Reports** | Balance Sheet, Profit & Loss (P&L), Trial Balance, General Ledger | Dynamic Reporting Engine (`account_reports` in Enterprise, SQL/QWeb in Community) | IFRS / US GAAP compliance, executive audits, tax authorities. |
| **Aging & Audit Reports** | Aged Receivables, Aged Payables, Partner Ledger | Reconciled Line Analysis (`account.move.line`) | Liquidity monitoring, expected credit loss (IFRS 9), auditor verification. |

### Statutory Compliance on Invoices (`account.report_invoice_document`)
When customizing invoice templates:
- Must display Legal Company Name, VAT / Tax ID, Fiscal Address.
- Must display Customer Name, VAT ID, and Delivery vs Invoicing Address.
- Tax lines must show the Statutory Tax Breakdown (Tax Base Amount + Tax Amount per rate), matching statutory requirements.
- Under electronic invoicing regimes (e.g. PEPPOL, Factur-X, ZATCA), the PDF must embed machine-readable XML (`factur-x.xml` attachment).
