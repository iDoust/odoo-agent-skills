...
### Modern Price Computation in Sale Order
In Odoo 17, 18, and 19, `sale.order.line` computes its unit price and discounts via computed methods rather than manual onchanges.

```python
class SaleOrderLine(models.Model):
    _inherit = 'sale.order.line'

    # In modern Odoo, price_unit is computed from the pricelist rule:
    # @api.depends('product_id', 'product_uom_qty', 'discount', 'order_id.pricelist_id')
    # def _compute_price_unit(self): ...

    def _get_display_price(self):
        """Standard hook for computing line unit price from order pricelist."""
        self.ensure_one()
...
## Accounting & Financial Integration (IFRS 15 & IAS 21)

Every price calculated by a pricelist flows upstream from sales into downstream statutory financial reporting:

### 1. IFRS 15: Transaction Price & Trade Discounts
- **Trade Discounts (Pricelist Reductions)**: Under IFRS 15 (Revenue from Contracts with Customers), trade discounts and volume rebates directly reduce the transaction price allocated to performance obligations.
- **Journal Entries on Invoicing (`sale.order._create_invoices()`)**:
  Trade discounts are NOT booked as a separate expense. The line revenue is recognized strictly net of trade discounts:
  - **Dr. Accounts Receivable (`asset_receivable`)**: Total invoice amount inclusive of tax.
  - **Cr. Sales Revenue (`income`)**: `price_subtotal` (Net of pricelist discount: `price_unit * (1 - discount/100) * quantity`).
  - **Cr. Tax Payable (`liability_current`)**: Calculated on net taxable base (`price_subtotal`).

### 2. Settlement / Cash Discounts vs Trade Discounts
- **Cash Discounts (Early Payment)**: Configured in `account.payment.term` (e.g., 2/10 Net 30).
- When a customer pays within the discount window, Odoo reconciliation clears the full receivable and posts the discount difference to a dedicated Cash Discount Loss account (`expense_other`) or reduces revenue according to national GAAP.

### 3. IAS 21: Multi-Currency Pricelists
- If a customer has a pricelist in foreign currency (e.g., EUR) while the company functional currency is USD:
  - The transaction is recorded in foreign currency (`amount_currency`) and translated to USD at the exchange spot rate on the invoice accounting date (`date`).
  - Upon payment settlement (`account.payment`), any variance between invoice exchange rate and payment receipt rate is automatically recognized as **Realized FX Gain/Loss** (`income_other` / `expense_other`).
