...
def _cron_check_low_stock(self):
    ...
    # Search storable products with version-aware domain:
    # In v18+: ('is_storable', '=', True)
    # In v17:  ('detailed_type', '=', 'product')
    storable_domain = (
        [('is_storable', '=', True)]
        if 'is_storable' in self.env['product.template']._fields
        else [('detailed_type', '=', 'product')]
    )
...
## Stock Valuation & Accounting Integration (IAS 2)

Under international accounting standards (IAS 2 - Inventories), physical stock movements must reconcile with balance sheet asset valuation. In Odoo, this bridge is provided by `stock_account`:

### Automated (Perpetual) Inventory Valuation
When a product category has `property_valuation == 'real_time'`:
1. **Goods Receipt (Vendor Delivery)**:
   - Debit: **Stock Valuation Account** (`asset_current`)
   - Credit: **Stock Interim (Received) Account** (`asset_current`)
   - When Vendor Bill arrives: Debit Stock Interim (Received), Credit Accounts Payable (`liability_payable`).
2. **Goods Issue (Customer Delivery)**:
   - Debit: **Cost of Goods Sold (COGS)** (`expense_direct_cost`)
   - Credit: **Stock Valuation Account** (`asset_current`)
3. **Landed Costs**:
   - Costs such as freight, customs, and insurance (`stock.landed.cost`) are allocated onto received stock lines, updating valuation layers and posting valuation adjustments.

> **Integrity Rule**: Never bypass stock moves by directly executing SQL on `stock_quant` or `stock_valuation_layer`. Doing so corrupts the financial balance between the general ledger and warehouse inventory.
...
8. **Storable product filtering** - Use `is_storable` in v18+ (`detailed_type` in v17)
9. **Accounting alignment** - Use `stock_account` automated valuation for financial compliance
10. **Concurrency** - Use proper locking (`_acquire_record_to_lock` or `SELECT FOR UPDATE`) for inventory updates
