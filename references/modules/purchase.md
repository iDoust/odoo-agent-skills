...
## End-to-End Accounting Pipeline: Procure-to-Pay (P2P)

In standard Odoo ERP, procurement flows from initial vendor quotation into the general ledger and financial reports through the Procure-to-Pay (P2P) lifecycle:

```
[Request for Quotation (draft)]
        │  button_confirm()
        ▼
[Purchase Order (purchase)] ──► [Goods Receipt (stock.picking)]
        │                                  │
        │                                  ▼ button_validate()
        │                           Stock Valuation Entry (IAS 2)
        │                           Dr: Stock Valuation (asset_current)
        │                           Cr: Stock Interim Received (asset_current)
        │
...
Dr: Stock Interim Received (or Expense)
Dr: Tax Receivable (asset_current)
Cr: Accounts Payable (liability_payable)
        │
        ▼ register_payment()
[Vendor Payment (account.payment)]
Dr: Accounts Payable (liability_payable)
Cr: Outstanding Payments (asset_current)
        │
        ▼ bank reconciliation
Bank Clearing
Dr: Outstanding Payments (asset_current)
Cr: Bank Account (asset_cash)
        │
        ▼
[Financial Reports: Balance Sheet, Aged Payables, Tax Report, Stock Valuation]
```

### 1. 3-Way Matching & Bill Control Policy (`purchase_method`)
Odoo enforces audit-proof 3-way matching across purchase lines, receipts, and bills through `product.template.purchase_method`:
- **`receive` (Received Quantities - Standard for Storable Goods)**: Vendor bills can only be generated for quantities physically received at the warehouse (`qty_received > qty_invoiced`). Prevents paying for missing shipments.
- **`purchase` (Ordered Quantities - Services / Direct Expenses)**: Billed directly upon PO confirmation (`product_qty > qty_invoiced`).

```python
# Programmatic vendor bill generation from confirmed purchase order
def _generate_vendor_bill(self, order):
    """Generate and post vendor bill for received lines."""
    if order.invoice_status != 'to invoice':
        return self.env['account.move']

    # Standard Odoo core method to generate bill moves
    bills = order.action_create_invoice()
    bills.action_post()
    return bills
```

### 2. Downstream Financial Effects
1. **Balance Sheet**:
   - Inventory Asset debited via `stock_account` upon receipt.
   - Accounts Payable (`liability_payable`) credited upon bill posting.
   - Payment registration clears AP into Outstanding Payments, and bank statement reconciliation clears it from Cash/Bank (`asset_cash`).
2. **Profit & Loss**:
   - For consumable goods/services, expenses are recognized upon bill posting.
   - For storable goods, costs remain capitalized in inventory assets until sold (COGS).
3. **Statutory Reports**:
   - Input VAT credited against tax obligations on the Tax Report.
   - Aged Payables report tracks liabilities by due date (`date_maturity`).
...
3. **3-Way matching enforcement** - Enforce `purchase_method='receive'` for storable products
4. **Multi-company** - Handle inter-company purchases with `check_company=True`
...
8. **Invoicing integrity** - Always use `order.action_create_invoice()`; never create detached bills manually
9. **Accounting alignment** - Reconcile Stock Interim Received account to ensure zero unbilled receipt discrepancy
10. **Testing** - Test full RFQ → PO → receipt → bill → payment reconciliation flow
