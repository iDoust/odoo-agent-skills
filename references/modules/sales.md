...
## End-to-End Accounting Pipeline: Order-to-Cash (O2C)

In standard Odoo ERP, sales operations flow from customer inquiry into the general ledger and financial reports through the Order-to-Cash (O2C) lifecycle:

```
[Quotation (draft)]
        │  action_confirm()
        ▼
[Sales Order (sale)] ──────► [Stock Delivery (stock.picking)]
        │                                  │
        │                                  ▼ button_validate()
        │                           COGS Journal Entry (IAS 2)
...
Dr: Accounts Receivable (asset_receivable)
Cr: Sales Revenue (income)
Cr: Tax Payable (liability_current)
        │
        ▼ register_payment()
[Customer Payment (account.payment)]
Dr: Outstanding Receipts (asset_current)
Cr: Accounts Receivable (asset_receivable)
        │
        ▼ bank reconciliation
Bank Clearing
Dr: Bank Account (asset_cash)
Cr: Outstanding Receipts (asset_current)
        │
        ▼
[Financial Reports: P&L, Balance Sheet, Aged Receivables, Tax Report]
```

### 1. Invoicing Policy & 3-Way Matching (`invoice_policy`)
Odoo enforces revenue recognition rules through `product.template.invoice_policy`:
- **`delivery` (Delivered Quantities - Default for Storable)**: Lines can only be invoiced once goods are delivered (`qty_delivered > qty_invoiced`). Aligns with IFRS 15 performance obligations.
- **`order` (Ordered Quantities - Services/Prepayments)**: Invoiced immediately upon sales order confirmation (`product_uom_qty > qty_invoiced`).

```python
# Programmatic invoice generation from confirmed sales order
def _generate_order_invoice(self, order):
    """Generate and post invoice for invoiceable lines."""
    if order.invoice_status != 'to invoice':
        return self.env['account.move']

    # Standard Odoo core method to generate invoice moves
...
1. **Income Statement (P&L)**:
   - Sales revenue credited to `income` accounts based on product or product category.
   - Cost of Goods Sold debited to `expense_direct_cost` upon stock delivery.
2. **Balance Sheet**:
   - Accounts Receivable (`asset_receivable`) tracks unpaid customer debt.
   - Payment registration clears AR into Outstanding Receipts, and bank statement reconciliation clears it into Cash/Bank (`asset_cash`).
3. **Statutory Reports**:
   - VAT/Tax Declaration populated via `account.tax` mappings on invoice lines.
   - Aged Receivables analysis populated by `date_maturity` and `amount_residual`.
...
8. **Invoicing integrity** - Always use `order._create_invoices()`; never create unlinked invoices manually
9. **Accounting alignment** - Match invoice policies to IFRS 15 revenue recognition requirements
10. **Testing** - Test full quotation → order → delivery → invoice → payment reconciliation flow
