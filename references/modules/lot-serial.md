...
# In Odoo 18+, storable tracked products use is_storable=True (type is 'consu').
...
## Inventory Valuation & Accounting for Tracked Lots/Serials (IAS 2)

Traceability is not merely warehouse logistics; it directly drives financial balance sheet accuracy under IAS 2:

### 1. Specific Identification Costing vs Average Cost
- Under IAS 2 (Inventories, paragraph 23), items that are segregated for specific projects or not ordinarily interchangeable must have their costs assigned by specific identification.
- In Odoo, tracking by serial or unique lot numbers ensures each individual receipt links to a specific `stock.valuation.layer` entry with its exact purchase cost, preventing cost distortion across batches.

### 2. Expired Lots, Spoilage & Scrapping Accounting
Holding expired, unsaleable lots in inventory overstates assets and violates the IAS 2 rule requiring inventory to be stated at the **Lower of Cost and Net Realizable Value (NRV)**.
- **Scrap Operation (`stock.scrap`)**:
  When a batch expires or fails QC, creating a scrap order transfers the stock from the internal location to `virtual_location/scrap`.
- **Statutory Accounting Entry**:
  - **Dr. Loss on Inventory Write-down / Scrap Expense (`expense`)**: Full acquisition or manufactured cost of the scrapped lot.
  - **Cr. Stock Valuation Account (`asset_current`)**: Removed from perpetual balance sheet asset.
- **Traceability in Audit**:
  Auditors can trace from the financial ledger entry (`account.move.line`) -> `stock.valuation.layer` -> `stock.move` -> `stock.move.line` -> `stock.lot` -> original vendor bill or production order.
