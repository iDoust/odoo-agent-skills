...
**Version Notes:**
- Example targets Odoo 18.0
- Verify model, view, and API details against the target checkout

**Upgrade Notes:**
- Compare the source and target version references before upgrading
    """,
    'author': 'My Company',
    'website': 'https://mycompany.com',
    'license': 'LGPL-3',
...
class EquipmentAsset(models.Model):
    ...
    # Enable automatic company validation when the target model requires it
    ...
    assigned_employee_id = fields.Many2one(
        ...
        tracking=True,
        check_company=True,  # Automatic company validation when required
    )
    ...
    def action_view_maintenance(self) -> dict[str, Any]:
        ...
        return {
            ...
            'view_mode': 'list,form',  # 'tree,form' in v17
            'domain': [('equipment_id', '=', self.id)],
            'context': {'default_equipment_id': self.id},
        }

    # === SQL OPERATIONS (verify the target SQL API) === #
...
<odoo>
    ...
    <!-- List View (in v18+ use <list>; in v17 use <tree>) -->
    <record id="equipment_asset_view_list" model="ir.ui.view">
        <field name="name">equipment.asset.list</field>
        <field name="model">equipment.asset</field>
        <field name="arch" type="xml">
...
<odoo>
    ...
    <!-- Multi-Company Rule (verify the target context variable) -->
