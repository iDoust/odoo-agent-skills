...
# In v17: use detailed_type='product'
# In v18+: use is_storable=True (type defaults to 'consu')
template = self.env['product.template'].create({
    'name': 'T-Shirt',
    'is_storable': True,  # v18+ storable tracking; use detailed_type='product' in v17
...
# In Odoo 18+, storable products use is_storable=True (type is 'consu').
# In Odoo 17, use detailed_type='product'.
template = self.env['product.template'].create({
    'name': 'T-Shirt',
    'is_storable': True,  # v18+; in v17: 'detailed_type': 'product'
    'attribute_line_ids': [
        Command.create({
            'attribute_id': color_attr.id,
            'value_ids': [Command.set(colors.ids)],  # Red, Blue, Green
        }),
        Command.create({
            'attribute_id': size_attr.id,
            'value_ids': [Command.set(sizes.ids)],  # S, M, L, XL
        }),
    ],
})
    ...
    def _onchange_product_template(self):
        ...
        for attr_line in self.product_template_id.attribute_line_ids:
            lines.append(Command.create({
                ...
                'value_ids': [Command.set(attr_line.value_ids.ids)],
            }))
...
<record id="product_template_tshirt" model="product.template">
    ...
    <!-- In v18+: <field name="is_storable">True</field> (type is 'consu').
         In v17: <field name="detailed_type">product</field> -->
    ...
    <field name="attribute_line_ids" eval="[
        Command.create({
            ...
            'value_ids': [Command.set([ref('product_attribute_value_red'), ref('product_attribute_value_blue')])],
        }),
    ]"/>
</record>
```
...
## Statutory Accounting & Financial Classification (IAS 2 vs Operating Expense)

The configuration of a product in Odoo directly dictates its statutory accounting treatment from procurement through sale:

| Product Classification | Odoo Configuration | Balance Sheet Treatment | P&L Income Statement Treatment | Standard |
|------------------------|--------------------|-------------------------|--------------------------------|----------|
| **Storable Product** | `is_storable = True` (v18+) / `detailed_type = 'product'` (v17) | **Asset**: Stock Valuation Account (`asset_current`) held on Balance Sheet until shipped. | **COGS**: Recognized on customer delivery: `Dr. Expense COGS` / `Cr. Stock Interim (Delivered)`. | IAS 2 (Inventories) |
| **Consumable Product** | `type = 'consu'`, `is_storable = False` | **None**: No stock valuation asset tracked on Balance Sheet. | **Operating Expense**: Expensed directly to expense account on vendor bill posting. | Standard OPEX |
| **Service Product** | `type = 'service'` | **None**: Zero stock moves generated. | **Direct Revenue / Expense**: Billed or expensed directly to Income/Expense accounts. | IFRS 15 |

### Account Configuration on Categories (`product.category`)
Never hardcode accounts on individual product lines when possible; let them inherit from `categ_id`:
- `property_account_income_categ_id`: Credited on Customer Invoices (`account_type == 'income'`).
- `property_account_expense_categ_id`: Debited on Vendor Bills or COGS entries (`account_type == 'expense'` or `'expense_direct_cost'`).
- `property_stock_valuation_account_id`: Asset account for perpetual inventory (`asset_current`).
- `property_stock_account_input_categ_id`: Interim clearing for goods received not yet billed.
- `property_stock_account_output_categ_id`: Interim clearing for goods delivered not yet invoiced.
