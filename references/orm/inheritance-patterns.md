...
> [!TIP]
> For standard core Odoo mixins (`mail.thread`, `mail.activity.mixin`, `image.mixin`, `avatar.mixin`, `utm.mixin`, `rating.mixin`, `portal.mixin`), see [mixins.md](mixins.md).
...
<odoo>
    <record id="view_partner_form_inherit" model="ir.ui.view">
        ...
        <field name="arch" type="xml">
            ...
            <!-- Hide field with the target view's inline expression syntax -->
...
**Example:**
The available view types for a model can differ by version, edition, and
installed modules. Attempting to extend a view that is not present causes a
runtime error, so always verify the XML ID and view type first.
...
# Before extending: check if the view type exists for the target database.
# available_views = check_available_views('my.model')
```
...
**Example:**
A kanban view may use a QWeb card template rather than the element hierarchy
you expect. Read the target view before writing the XPath.
...
**Example:**
If the target kanban view uses `<t t-name="card">`, an XPath matching a
guessed `<div>` hierarchy will fail. Match the structure actually present in
the target view.
...
```xml
<!-- Inline expression syntax; verify the target view architecture -->
