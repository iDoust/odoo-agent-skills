...
state = fields.Selection(
    ...
    ondelete={                          # Handle deletion of added values
...
partner_id = fields.Many2one(
    comodel_name='res.partner',
    string='Partner',
    check_company=True,                # Use when company isolation requires it
)
```
...
## Relational Operations: Command API (x2many)

In modern Odoo (v17, v18, v19), always use the `Command` namespace (`from odoo.fields import Command`) when manipulating `One2many` and `Many2many` fields in `create()` and `write()`. Do not rely on legacy magic integer tuples (`(0, 0, ...)`, `(6, 0, ...)`).

| Command Method | Legacy Tuple | Effect |
|---|---|---|
| `Command.create(values)` | `(0, 0, values)` | Creates a new record with `values` and links it. |
| `Command.update(id, values)` | `(1, id, values)` | Updates record `id` with `values`. |
| `Command.delete(id)` | `(2, id, 0)` | Removes link AND deletes record `id` from the database. |
| `Command.unlink(id)` | `(3, id, 0)` | Removes link to `id` without deleting the record. |
| `Command.link(id)` | `(4, id, 0)` | Links existing record `id`. |
| `Command.clear()` | `(5, 0, 0)` | Unlinks all records (clears the relation). |
| `Command.set(ids)` | `(6, 0, ids)` | Replaces existing links with precisely the given `ids`. |
...
record = self.env['sale.order'].create({
    ...
    'order_line': [
        Command.create({'product_id': prod_1.id, 'product_uom_qty': 2.0}),
        Command.create({'product_id': prod_2.id, 'product_uom_qty': 5.0}),
    ],
    'tag_ids': [Command.set([tag_vip.id, tag_retail.id])],
})

# In write()
record.write({
    'order_line': [
        Command.update(line_1.id, {'product_uom_qty': 10.0}),
        Command.delete(line_2.id),
        Command.create({'product_id': prod_3.id, 'product_uom_qty': 1.0}),
    ],
    'tag_ids': [
        Command.link(tag_urgent.id),
        Command.unlink(tag_retail.id),
    ],
})

# Reset / clear all relations
record.write({
...
| `tracking` | bool | Track in chatter |
