...
## Domain Helper Functions & Version Differences

### Odoo 19: AST `Domain` Object (`from odoo.fields import Domain`)

In **Odoo 19**, `odoo.osv` and `odoo.osv.expression` are deprecated (`warnings.warn("Since 19.0, odoo.osv is deprecated use odoo.fields.Domain")`).
Odoo 19 introduces an immutable AST `Domain` class re-exported in `odoo.fields`:

```python
from odoo.fields import Domain

# 1. Building domains with Pythonic bitwise operators
domain_draft = Domain('state', '=', 'draft')
domain_amount = Domain('amount_total', '>', 1000)

# AND operator (&)
combined_and = domain_draft & domain_amount

# OR operator (|)
combined_or = domain_draft | Domain('state', '=', 'sent')

# NOT operator (~)
not_cancelled = ~Domain('state', '=', 'cancel')

# 2. Combining collections of domains
all_active = Domain.AND([
    Domain('active', '=', True),
    Domain('company_id', '=', self.env.company.id),
    domain_amount,
])
...
# 3. Boolean constants
always_true = Domain.TRUE    # Evaluates to (1, '=', 1)
always_false = Domain.FALSE  # Evaluates to (0, '=', 1)

# 4. Parsing legacy list-of-tuples domains
legacy_list = [('state', '=', 'draft'), ('active', '=', True)]
ast_domain = Domain(legacy_list)

# 5. Converting AST Domain back to list (for legacy APIs or external callers)
domain_list = list(combined_and)

# 6. Searching models using Domain objects directly
records = self.env['my.model'].search(combined_and)
```

### Odoo 17 & 18: `expression` Utilities (`from odoo.osv import expression`)

In **Odoo 17 and 18**, domains are manipulated as prefix-notation lists using the `odoo.osv.expression` module:
...
class DomainHelper(models.AbstractModel):
    ...
    def combine_domains(self, *domains):
        """Combine multiple domains with AND (Odoo 17/18)."""
        return expression.AND(list(domains))

    def combine_domains_or(self, *domains):
        """Combine multiple domains with OR (Odoo 17/18)."""
...
# AND combination: [('state', '=', 'draft'), ('user_id', '=', uid)]
combined = expression.AND([domain1, domain2])

# OR combination: ['|', ('state', '=', 'draft'), ('user_id', '=', uid)]
