...
### Transaction Isolation: Savepoints & Isolated Cursors

> [!WARNING]
> **Official Odoo Coding Guideline:** Never call `cr.commit()` or `cr.rollback()` directly on `self.env.cr`!
> Doing so causes partial commits, breaks test rollback isolation, and can leave the database in an inconsistent state.

#### Pattern A: Sub-Transaction Isolation (`cr.savepoint()`)
Use `with self.env.cr.savepoint():` when processing a batch of records where one failure shouldn't abort the entire transaction:

```python
def process_records_safely(self):
    """Process records with savepoints to isolate individual failures."""
    failed_records = []
    for record in self:
        try:
            with self.env.cr.savepoint():
            ...
            _logger.exception("Failed processing record %s: %s", record.display_name, e)
    ...
    if failed_records:
        self.env['bus.bus']._sendone(
            ...
            'simple_notification',
            {
                'title': 'Batch Processing Notice',
                'message': f"{len(failed_records)} records failed to process.",
                'type': 'warning',
            }
        )
    return True
...
#### Pattern B: Long-Running Batch Jobs / Crons (Isolated Cursor)
When a cron job or background worker processes thousands of records and needs periodic commits, create an explicit separate cursor from the registry/pool:

```python
def process_large_batch_cron(self):
    """Process high-volume batch using an isolated, explicitly owned cursor."""
    record_ids = self.search([('state', '=', 'pending')]).ids

    # Step 1: Open isolated cursor
    with self.pool.cursor() as new_cr:
        new_env = api.Environment(new_cr, self.env.uid, self.env.context)
        batch = new_env['my.model'].browse(record_ids[:500])

        for record in batch:
            record._process()

        # Step 2: Safe commit on explicitly owned cursor only
        new_cr.commit()
```
