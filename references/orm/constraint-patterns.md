...
The declaration API is version-sensitive. Check the matching file under
`references/versions/` and the target source before copying either example;
the SQL expressions themselves are generally stable.

### Odoo 19+ (and SaaS 18.1+): `models.Constraint` declaration
```python
...
### Odoo 17 & 18.0 LTS: `_sql_constraints` declaration
...
### Common SQL constraint expressions
