# Odoo Security Guide - Shared Core

This document covers security concepts shared by Odoo 17–19. For
...
# ✓ SAFE: Access check performed (Odoo 18 & 19 unified check)
record = self.env['my.model'].browse(user_provided_id)
record.check_access('read')  # Validates both ACLs and record rules in one call
return record.sensitive_data

# ✓ SAFE in Odoo 17 (separate ACL and record rule checks):
...
**Note**: This document is the shared 17–19 baseline. Verify syntax and
behavior against the target version reference before applying it.
