...
# Worker function kept at module scope for executor compatibility.
def _parse_file_worker(file_path: Path, module_name: str) -> tuple[Path, list[dict]]:
    """Worker function to parse a file in the executor.
...
class OdooIndexer:
    ...
    async def index_all_modules(self, incremental: bool = True, module_filter: Optional[list[str]] = None):
        ...
        # Create a persistent thread pool for file parsing.
    ...
    async def index_module(self, module_name: str, module_path: Path, incremental: bool = True):
        ...
        try:
            ...
            parse_tasks = [
                loop.run_in_executor(self.executor, _parse_file_worker, file_path, module_name)
                for file_path in files_to_index
            ]
            try:
